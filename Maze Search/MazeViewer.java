...
public class MazeViewer {
   ...
   */
   private static MazeFrame readMazeFile(String fileName) throws IOException {
	   ...
	   MazeCoord[] start_end = new MazeCoord[2];
	   ...
	   if(in.hasNextLine()){
		   ...
		   rows = Integer.parseInt(ln.next());
		   columns = Integer.parseInt(ln.next());
	   }
	   
	   boolean[][] maze_data = new boolean[rows][columns];
	   
	   for(int i=0;i<rows+2;i++){
		   //String s2 = in.nextLine();
		   Scanner l1 = new Scanner(in.nextLine());
		   if(i>=rows){
			   start_end[count] = new MazeCoord(Integer.parseInt(l1.next()),Integer.parseInt(l1.next()));
			   count++;
		   }
		   else{
			   l1.useDelimiter("");
      ...
      return new MazeFrame(maze_data, start_end[0], start_end[1]);
